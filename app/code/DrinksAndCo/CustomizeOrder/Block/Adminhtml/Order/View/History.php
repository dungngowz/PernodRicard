 ...
 */
namespace DrinksAndCo\CustomizeOrder\Block\Adminhtml\Order\View;
...
class History extends \Magento\Backend\Block\Template
{
    ...
    public function __construct(
        ...
        \DrinksAndCo\Agent\Model\InfomationFactory $agentFactory,
        array $data = []
    ) {
    ...
    {
        $collection = $this->agentFactory->create();
        $records = $collection->getCollection()->toArray();
        $data = [''];
        
        if($records['items']){
            foreach($records['items'] as $item){
                $data[$item['infomation_id']] = $item['name'];
            }
        }

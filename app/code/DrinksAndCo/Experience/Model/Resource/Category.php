...
class Category extends AbstractDb
{
    ...
    protected function _beforeDelete(AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('DrinksAndCo_Experience_category_store'), $condition);
        $this->getConnection()->delete($this->getTable('DrinksAndCo_Experience_category_post'), $condition);
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $table = $this->getTable('DrinksAndCo_Experience_category_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['DrinksAndCo_Experience_category_store' => $this->getTable('DrinksAndCo_Experience_category_store')],
                $this->getMainTable() . '.category_id = DrinksAndCo_Experience_category_store.category_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'DrinksAndCo_Experience_category_store.store_id IN (?)',
                $storeIds
            )->order(
                'DrinksAndCo_Experience_category_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('DrinksAndCo_Experience_category_store')],
    ...
    public function lookupStoreIds($categoryId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('DrinksAndCo_Experience_category_store'),
            'store_id'
        )->where(
            'category_id = ?',
            (int)$categoryId
    ...
    public function checkUrlKeyExits(AbstractModel $object)
    {
        ...
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('DrinksAndCo_Experience_category'),
            'category_id'
        )
            ->where(
                'url_key = ?',

...
namespace DrinksAndCo\Experience\Block\Post;
...
class View extends \Magento\Framework\View\Element\Template
{
    ...
    protected function _addBreadcrumbs()
    {
        ...
        $breadcrumbsBlock->addCrumb(
            ...
            [
                'label' => $pageTitle,
                'title' => $pageTitle,
                'link' => $this->_experienceHelper->getRoute()
            ]
        );
        if ($category != false) {
            $breadcrumbsBlock->addCrumb(
    ...
    protected function _prepareLayout()
    {
        ...
        $this->pageConfig->addBodyClass('experience-post-view');
        if ($pageTitle) {
		...
		if($post->getGalleryImage()){
			...
			if(count($galleryArray)>0){
				foreach($galleryArray as $img){
					$filePath = 'DrinksAndCo_Experience/gallery/image'.$img;
					if($filePath!=''){

...
namespace DrinksAndCo\Region\Controller\Adminhtml\Region;
...
class Edit extends \Magento\Backend\App\Action
{
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'DrinksAndCo_Region::region_edit';
    ...
    public function execute()
    {
        ...
        $model = $this->_objectManager->create('DrinksAndCo\Region\Model\Region');
        $model->setData([]);
        // Initial checking
        if ($id && $id > 0) {
            $model->load($id);
        ...
        $this->_coreRegistry->register('drinksandco_region', $model);

        $countryHelper = $this->_objectManager->get('Magento\Directory\Model\Config\Source\Country');

        $this->_coreRegistry->register('drinksandco_region_country_list', $countryHelper->toOptionArray());

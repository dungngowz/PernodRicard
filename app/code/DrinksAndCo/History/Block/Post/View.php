...
namespace DrinksAndCo\History\Block\Post;
...
class View extends \Magento\Framework\View\Element\Template
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->pageConfig->addBodyClass('history-post-view');
        if ($pageTitle) {
	...
	public function getUrlPostById($id,$value,$name){
		$arrResult = [];
		$route = $this->_historyHelper->getRoute();
		$route = $route . '/' . $value;
		...
		if($post->getGalleryImage()){
			...
			if(count($galleryArray)>0){
				foreach($galleryArray as $img){
					$filePath = 'DrinksAndCo_History/gallery/image'.$img;
					if($filePath!=''){

...
class Save extends History
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                $historyHelper = $this->_objectManager->create('DrinksAndCo\History\Helper\Data');
                ...
                if (isset($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        $path = $this->_objectManager->get('Magento\Framework\Filesystem')->getDirectoryRead(DirectoryList::MEDIA)
                            ->getAbsolutePath('ecg/history/');
                        $uploader->save($path);
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['thumbnail'] = 'ecg/history' . $fileName;
                        }
                    } else {
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['image'] = 'ecg/history' . $fileName;
                        }
                    } else {
                        $this->messageManager->addError(__('Disallowed file type.'));
                        return $this->redirectToEdit($model, $data);
                ...
                if (!isset($data['post']['url_key']) || $data['post']['url_key'] == '') {
                    ...
                    $existUrlKeys = $this->_objectManager->create('DrinksAndCo\History\Model\Post')->getCollection()->addFieldToFilter('url_key', ['eq' => $urlKey]);
                ...
                if (isset($data['post']['tags'])) {
                    ...
                    foreach ($tags as $key => $tag) {
                        $tags[$key] = $historyHelper->convertSlashes($tag, 'forward');
                    }
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('history/post/edit', ['post_id' => $model->getId()]);
                    return;
                }
                ...
                if (!empty($id)) {
                    $this->_redirect('history/post/edit', ['post_id' => $id]);
                } else {
                    $this->_redirect('history/post/new');
                }
                return;
            } catch (\Exception $e) {
                ...
                $this->_redirect('history/post/edit', ['post_id' => $this->getRequest()->getParam('post_id')]);
                return;
            }
        }
        $this->_redirect('history/post/index');

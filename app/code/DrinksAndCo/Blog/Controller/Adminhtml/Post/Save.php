...
class Save extends Blog
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                $blogHelper = $this->_objectManager->create('DrinksAndCo\Blog\Helper\Data');
                $model = $this->_objectManager->create('DrinksAndCo\Blog\Model\Post');
                ...
                if (isset($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        $path = $this->_objectManager->get('Magento\Framework\Filesystem')->getDirectoryRead(DirectoryList::MEDIA)
                            ->getAbsolutePath('drinksandco/blog/');
                        $uploader->save($path);
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['thumbnail'] = 'drinksandco/blog' . $fileName;
                        }
                    } else {
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['image'] = 'drinksandco/blog' . $fileName;
                        }
                    } else {
                        $this->messageManager->addError(__('Disallowed file type.'));
                        return $this->redirectToEdit($model, $data);
                ...
                if (!isset($data['post']['url_key']) || $data['post']['url_key'] == '') {
                    ...
                    $existUrlKeys = $this->_objectManager->create('DrinksAndCo\Blog\Model\Post')->getCollection()->addFieldToFilter('url_key', ['eq' => $urlKey]);
    ...
    {
        return $this->_authorization->isAllowed('DrinksAndCo_Blog::save_post');
    }
}

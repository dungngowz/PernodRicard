		...
		if($this->getRequest()->isAjax()){
			...
			if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
				...
				if(($file['name']!='') && ($file['size'] >0)){
					...
					$path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('drinksandco_blog/gallery/image/');
					$uploader->save($path);
					$fileName = $uploader->getUploadedFileName();
					if($this->isFile('drinksandco/blog/gallery/image/'.$fileName)){
